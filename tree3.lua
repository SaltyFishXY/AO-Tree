...
local function growTree(owner, timestamp)
    ...
    if tree.height >= 10 then
        ...
        -- 更新数据库中的用户代币数量
        dbAdmin.updateTokenCount(owner, tokenCount)  -- 假设 dbAdmin 中有这个方法
...
-- 更新用户代币数量的函数
local function updateTokenCountForUser(owner, newTokenCount)
    tokenCount = newTokenCount  -- 更新本地代币数量
    dbAdmin.updateTokenCount(owner, newTokenCount)  -- 更新数据库中的代币数量
end
